...
bool send_packets(Tox *tox, friend * f, TOX_ERR_FRIEND_CUSTOM_PACKET * error) {
    ...
    if (p->lossless) {
        rv = tox_friend_send_lossless_packet(tox, f->friend_number, p->data, p->lenght,
                                     error);
    } else {
...
void file_chunk_request(Tox *tox, uint32_t friend_number, uint32_t file_number, uint64_t position,
    ...
    if (f) {
        ...
        DL_FOREACH_SAFE(f->file_transfers, ft, tmp) {
            ...
            if (ft->file_number == file_number) {
                ...
                tox_file_send_chunk(tox, friend_number, file_number, position, ft->data+position,
                         length, &error);
    ...
    if (f->file_transfers) {
        ...
        DL_FOREACH_SAFE(f->file_transfers, ft, tmp) {
            ...
            if (ft->is_error) {
                ...
                tox_friend_send_message(tox, f->friend_number, TOX_MESSAGE_TYPE_NORMAL, (uint8_t*)msg, strlen(msg), NULL);
                tox_file_control(tox, f->friend_number, ft->file_number, TOX_FILE_CONTROL_CANCEL, NULL);
            ...
            if (ft->recv_complete && !ft->send_started) {
                ...
                ft->file_number = tox_file_send(tox, f->friend_number, TOX_FILE_KIND_DATA, 
                    ft->file_size, NULL, ft->filename, ft->filename_length, error);
                // check error or delete!!
                printf("Error in file send %d\n",*error);
                uint8_t * file_id = malloc(sizeof(uint8_t)*TOX_FILE_ID_LENGTH);
                TOX_ERR_FILE_GET error;
                tox_file_get_file_id(tox, f->friend_number, ft->file_number, file_id, &error);
...
static void *run_tox(void *arg)
{
    ...
    for (;;) {
        ...
        if (is_connected) {
            ...
            while (1) {
                ...
                if (elt->data_packets != NULL) {
                    ...
                    if (ret == false && error == TOX_ERR_FRIEND_CUSTOM_PACKET_FRIEND_NOT_CONNECTED) {
                        purge_packets(elt);
                ...
                us_diff = 1000000 * (tv_start.tv_sec-tv.tv_sec) + (tv_start.tv_usec-tv.tv_usec);
        ...
        if ((us_diff/1000) < time) {
            nanosleep((const struct timespec[]){{0, (time*1000-us_diff)* 1000L}}, NULL);
        }
...
void custom_lossy_friend_message(Tox *tox, uint32_t friend_number, const uint8_t *data, size_t length,
                                        void *user_data)
{
    ...
    if (f == NULL) {
        ...
        return;
    } else if ((f->data_packets_cached_bytes + length) > MAX_CUSTOM_PACKETS_CACHED_BYTES_PER_USER) {
        printf("Hit max transfer limit! Packets will be dropped\n");
        // TODO: notify the user!!
        return;
    }
...
void custom_lossless_friend_message(Tox *tox, uint32_t friend_number, const uint8_t *data, size_t length,
        void *user_data)
{
...
void friend_message(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, const uint8_t *message, size_t length, void *user_data)
{
	...
	if (!strcmp("!info", dest_msg)) {
		...
		const char *info_msg = "If you're experiencing issues, contact bonfus in #toktok at freenode";
...
file_transfer_t * init_file_transfer(uint32_t file_number, uint8_t* file_id, uint32_t kind, uint64_t file_size, const uint8_t *filename, size_t filename_length) {
...
void file_recv(Tox *tox, uint32_t friend_number, uint32_t file_number, uint32_t kind, uint64_t file_size, const uint8_t *filename, size_t filename_length, void *user_data)
{
    ...
    if (file_size > MAX_FILE_TRANSFER_SIZE) {
        ...
        const char *tmp = "Sorry, I don't accept files larger than %d bytes (and streaming not yet supported)" ;
        ...
        if (f == NULL) {
            printf("Hit transfer limit! Packets will be dropped\n");
            const char *tmp = "Hit server transfer limit (%d transfers)! File transfer will be dropped";
            char *msg = (char*) malloc(sizeof(char)*1000);
        ...
        ft = init_file_transfer(file_number, file_id, kind, file_size, filename, filename_length);
        DL_APPEND(f->file_transfers,ft);
        tox_file_control(tox, friend_number, file_number, TOX_FILE_CONTROL_RESUME, NULL);
    }
}

void file_recv_chunk(Tox *tox, uint32_t friend_number, uint32_t file_number, uint64_t position,
...
void file_recv_control(Tox *tox, uint32_t friend_number, uint32_t
 file_number, TOX_FILE_CONTROL control, void *user_data) {
    ...
    if (control == TOX_FILE_CONTROL_PAUSE || control == TOX_FILE_CONTROL_RESUME) {
        tox_file_control(tox, friend_number, file_number, TOX_FILE_CONTROL_RESUME, NULL);
